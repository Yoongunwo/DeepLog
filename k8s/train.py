...
preprocessor = Preprocessor(
    length  = 40,           # Extract sequences of 20 items
    timeout = float('inf'), # Do not include a maximum allowed time between events
)

# Load data from HDFS dataset
X_train       , y_train       , label_train       , mapping_train        = preprocessor.text("./k8s/data/train.txt"        , verbose=True)
X_test        , y_test        , label_test        , mapping_test         = preprocessor.text("./k8s/data/normal_test.txt"  , mapping=mapping_train, verbose=True)
X_test_anomaly, y_test_anomaly, label_test_anomaly, mapping_test_anomaly = preprocessor.text("./k8s/data/attack_test.txt", mapping=mapping_test, verbose=True)
...
max_index = max(mapping_test_anomaly.values()) + 1
deeplog = DeepLog(
    input_size  = max_index, # Number of different events to expect
    hidden_size = 64 , # Hidden dimension, we suggest 64
    output_size = max_index, # Number of different events to expect
)
deeplog.load("./k8s/deeplog_model.pth")
...
# deeplog.save("./k8s/deeplog_model.pth")
